<%@page import="Exceptions.FormException" %>
	...
	<body class="full-page flex flex-row">
		...
		<div class="form-container flex-1 flex flex-col align-center justify-center relative">
			...
			<% FormException ex=(FormException) request.getAttribute("formError"); if (ex==null) { ex=new
				FormException("no-error", "" ); } %>

				<form action="Login" method="post" class="login-form flex-1">
					<div class='form-field <%= ex.getElement() == "email" ? "error" : ""%>'>
						<span class="form-label">Email</span>
						<input type="text" class="form-input" name="email" id="email" value="${param[" email"]}" />
					...
					<div class='form-field <%= ex.getElement() == "password" ? "error" : ""%>'>
						...
						<div class="relative">
							<input style="width: 100%" type="password" class="form-input" name="password"
								id="password" />
